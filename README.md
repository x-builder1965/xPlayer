...
> **Note:** 変更履歴は、アプリ内ヘルプ（❔）下部の「変更履歴」をご参照ください。
...
- **📁 ローカル動画・音声再生**  
  ...
  - **対応フォーマット:**  
    ...
    - 音声：`mp3`, `wav`, `flac`, `ogg`, `oga`, `m4a`, `aac`, `opus`, `wma`, `aiff`, `aif`, `alac`, `ape` など  
  - HTML5 ネイティブ再生に対応する形式（`mp4`, `webm`, `ogg`, `mov`, `m4v`, 一部 `mkv`）はダイレクト再生  
  - 非対応形式は **FFmpeg** により背景で自動変換（日本語音声・字幕を優先取得）
...
- **⚙️ 設定・その他**  
  ...
  - キーボードショートカット対応（詳細はヘルプ（❔）を参照）

---

## 注意事項
- **多重起動について:** アプリの多重起動は可能ですが、最後に終了したウィンドウの設定値が保存されます。重複起動時にはコントロールパネル等の表示名頭に 🚫 マークが付与されます。
