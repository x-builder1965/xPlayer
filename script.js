...
// 🔲共通変数設定🔲
// モジュールインポート
const { ipcRenderer, fs, os, path, openVideoInBrowser, getFilePath, classifyPath, captureScreenshot, getVideoTracks } = window.electronAPI;
...
const languageMap = {
  ...
  'urd': 'ウルドゥー語',         // ur
  'vie': 'ベトナム語',           // vi
  'may': 'マレー語',      // マレー語（ms/may/msa）
  'cze': 'チェコ語',      // チェコ語（cs/cze/ces）
  'baq': 'バスク語',      // バスク語（eu/eus/baq）
  'kan': 'カンナダ語',    // カンナダ語（kn/kan）
  'mal': 'マラヤーラム語', // マラヤーラム語（ml/mal）
  'dut': 'オランダ語',    // オランダ語（nl/dut/nld）
  'tam': 'タミル語',      // タミル語（ta/tam）
  'tel': 'テルグ語',      // テルグ語（te/tel）
  'gre': 'ギリシャ語',    // ギリシャ語（el/gre/ell）
  'rum': 'ルーマニア語',     // ISO 639-2/B 版（ron と同義）
  // 追加：地域バリアント例（必要に応じて）
  'zh-cn': '中国語（簡体字）',
  'zh-tw': '中国語（繁体字）',
  'pt-br': 'ポルトガル語（ブラジル）',
...
async function playVideo(file) {
    ...
    videoPlayer.play().then(() => {
        ...
        }, 1000); // 1秒待機（トラック解析が遅い場合に対応）
    }).catch(err => {
        console.warn('再生開始失敗:', err);
        playPauseBtn.textContent = '▶️';
        playPauseBtn.setAttribute('data-tooltip', '再生（Space／Right Click）');
    });
...
function createSortMenu() {
    ...
    menu.addEventListener('mouseenter', () => {
        ...
        clearTimeout(timeout);           // 自動非表示タイマーを即時キャンセル
        showControlsAndFilename();       // 強制的に表示状態を維持
    });
    menu.addEventListener('mouseleave', () => {
        isMouseOverControls = false;
        // メニュー外に出たら通常の非表示タイマーを再開
        showControlsAndFilename();       // ここでタイマーが再セットされる
    });
...
function applySelectedSubtitle() {
    ...
    Array.from(videoPlayer.textTracks).forEach(track => {
        track.mode = (selectedSubtitle === '（なし）') 
            ? 'hidden'
            : (track.label === selectedSubtitle || track.language === selectedSubtitle)
                ? 'showing'
                : 'hidden';
    });
}
...
function selectTrack(type, label, menu, trackObj = null) {
    ...
    if (type === 'subtitle') {
        if (label === '（なし）') {
            ...
            if (trackObj) {
                currentTextTrack = trackObj;
            } else {
                // ラベルから逆引き（非推奨・複数あると危険）
                const found = currentTextTracks.find(t => remakeTagsTitle(t) === label);
                if (found) currentTextTrack = found;
            }
...
function createTrackMenu(type) {  // 'audio' or 'subtitle'
    ...
    const tracks = type === 'audio' ? currentAudioTracks : currentTextTracks;
    let selectedTrackObj = type === 'audio' ? currentAudioTrack : currentTextTrack;  // ← 重要: 現在再生中のトラックオブジェクト
    ...
    // ── 音声トラックの場合 ──
        ...
        if (tracks.length > 0) {
            ...
            tracks.forEach((track) => {
                let label = remakeTagsTitle(track);
                if (track.channels) {
...
audioTrackBtn.addEventListener('click', (e) => {
    ...
    toggleTrackMenu(e, 'audio', audioTrackBtn);
});

// 🔠字幕トラックボタンクリック
subtitleTrackBtn.addEventListener('click', (e) => {
    if (modeChange !== 'video') return;
    toggleTrackMenu(e, 'subtitle', subtitleTrackBtn);
});
