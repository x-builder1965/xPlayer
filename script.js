...
// 　・ポップアップメニューには「（なし）／動画パス▲／動画パス▼／作成日時▲／作成日時▼／（ランダム）」を配置。
// 　　・（なし）：main.jsで読み込まれた順
// 　　・動画パス▲／動画パス▼：動画ファイルのファイルパス順
// 　　・作成日時▲／作成日時▼：動画ファイルの作成日時順
// 　　・（ランダム）：ランダム再生順
// 　・ポップアップメニューは任意（メニュー以外）のクリックで非表示。
// 　・選択された並び順の前にチェック（✅）を表示。
...
editSeekBar.addEventListener('change', () => {
    ...
    if (lastUserSetTime !== null) {
        videoPlayer.currentTime = lastUserSetTime; // 念のため最後に1回入れる人もいる
        updateTimeDisplay();                       // 正しい時間で更新
        localStorage.setItem('currentTime', lastUserSetTime);
        lastUserSetTime = null;
    }
});
